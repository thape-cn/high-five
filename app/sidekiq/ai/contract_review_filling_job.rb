class AI::ContractReviewFillingJob
  ...
  def perform(contract_review_id, field_name)
    ...
    response = dify_chat.ask "合同审查", with: contract_review.contract_basic.upload_file_id
    Rails.logger.info "log in ContractBasicFillingJob #{field_name} with #{contract_review.contract_basic.upload_file_id}: #{response.content}"
